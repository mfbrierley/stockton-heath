...
| Discounts (`discounts.tsx`)                          | Approved, paid-for business discounts from `GET /business-listings`, searchable by business name or discount wording, and filterable by category chips built from the listings themselves. Hidden on the `production` EAS channel by `utils/discountsVisible.ts` until sign-off |
